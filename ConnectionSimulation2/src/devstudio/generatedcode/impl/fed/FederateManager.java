...
import devstudio.generatedcode.HlaTimeStampFactory;
import devstudio.generatedcode.HlaTuning;
import devstudio.generatedcode.exceptions.*;
import devstudio.generatedcode.impl.HlaLogicalTimeImpl;
import devstudio.generatedcode.impl.HlaWorldImpl;
import devstudio.generatedcode.impl.utils.MapAdapter;
import devstudio.generatedcode.impl.utils.NameValueMap;
import devstudio.generatedcode.impl.HlaFederateIdImpl;
...
public final class FederateManager extends FederateAmbassadorAdapter {
   ...
   public HlaLogicalTime connect(HlaSettings settings)
      ...
      try {
         ...
         if (profile != null && profile.length() == 1) {
            ...
            if ((rtiFactory.getRtiInfo().getHlaVersion() == HlaVersion.HLA_EVOLVED ||
                 rtiFactory.getRtiInfo().getHlaVersion() == HlaVersion.NONE) &&
   ...
   private se.pitch.ral.api.LogicalTime enableTimeManagement(boolean regulating, boolean constrained, boolean destroyFederationOnDisconnect, boolean onJoin) throws HlaInternalException, HlaInvalidLogicalTimeException, HlaRtiException, HlaNotConnectedException {
      ...
      try {
         ...
         throwException(destroyFederationOnDisconnect, new HlaInternalException(exceptionMessage, e), onJoin);
      } catch (RequestForTimeConstrainedPending e) {
         throwException(destroyFederationOnDisconnect, new HlaInternalException(exceptionMessage, e), onJoin);
      } catch (RequestForTimeRegulationPending e) {
         throwException(destroyFederationOnDisconnect, new HlaInternalException(exceptionMessage, e), onJoin);
      } catch (InvalidLookahead e) {
         throwException(destroyFederationOnDisconnect, new HlaInvalidLogicalTimeException(exceptionMessage, e), onJoin);
         ...
         throwException(destroyFederationOnDisconnect, new HlaSaveInProgressException(exceptionMessage, e), onJoin);
      } catch (RestoreInProgress e) {
         throwException(destroyFederationOnDisconnect, new HlaRestoreInProgressException(exceptionMessage, e), onJoin);
      } catch (RTIinternalError e) {
         throwException(destroyFederationOnDisconnect, new HlaRtiException(exceptionMessage, e), onJoin);
      } catch (FederateNotExecutionMember e) {
         throwException(destroyFederationOnDisconnect, new HlaNotConnectedException(exceptionMessage, e), onJoin);
      } catch (NotConnected e) {
         throwException(destroyFederationOnDisconnect, new HlaNotConnectedException(exceptionMessage, e), onJoin);
      }
      return currentTime;
   ...
   private <T extends Throwable> void throwException(boolean destroyFederationOnDisconnect, T exception, boolean onJoin) throws T {
      if (onJoin) {
   ...
   private HlaLogicalTime decodeLogicalTime(se.pitch.ral.api.LogicalTime logicalTime, final OrderType receivedOrdering) {
      if (logicalTime == null || !(logicalTime instanceof LogicalTimeInteger64)) {
   ...
   @Override
   public void announceSynchronizationPoint(final String synchPointLabel, byte[] userSupppliedTag) {
